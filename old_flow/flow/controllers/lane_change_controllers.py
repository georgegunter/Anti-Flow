...
class AILaneChangeController(BaseLaneChangeController):
    ...
    def get_lane_change_action(self, env):
        """See parent class."""
        return self.get_insentive_LC(env)


    def get_insentive_LC(self, env):
...
class I24_routing_LC_controller(BaseLaneChangeController):
    '''
    This LC controller is specifically for getting vehicles to change out of the 
    on-ramp merging lane into the freeway on the I24 network.
    ...
    def get_lane_change_action(self,env):
        ...
        return self.get_routing_LC_decision(env)

    def get_routing_LC_decision(self,env):
    ...
    def check_safe_merge_left(self,env):
        ...
        if(left_lane_follower_acc < -self.delta_follower):
            if(self.display_warning_messages):
                print('Vehicle '+self.veh_id+' cannot merge due to follower needing to brake at: '+str(left_lane_follower_acc))
            safe_to_merge = False
...
class I24_LC_controller(BaseLaneChangeController):
    ...
    def get_lane_change_action(self,env):
        '''
        Looks both at what the routing action is and what the insentive action is.
        The routing action takes precedence, and after checks for insentive if no
        ...
        insentive_LC = self.get_insentive_LC(env)
    ...
    def get_insentive_LC(self, env):
        """
        Implements the Rutger's MOBIL inspired lane-change algorithm for insentive
        based lane-changing.
        """
        # Increment how long since last lane-change:
        self.cooldown_period += env.sim_step
        ...
        if(self.cooldown_period >= self.switching_threshold):
            ...
            if (r_lane is not None):
                ...
                if((ego_edge == 'Eastbound_7') & (ego_lane == 1)):
