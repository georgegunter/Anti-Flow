...
def get_flow_params(attack_duration,
	...
	if(ACC_comp_params is not None):
		...
		adversary_accel_controller = (ACC_comp_inject_radar,{
			'k_1':k_1,
			'k_2':k_2,
			'h':h,
			'd_min':d_min,
...
if __name__ == '__main__':
	...
	if(want_random_sample):
		...
		attack_magnitude_range = [-1,3.0]
		attack_duration_range = [0,20]
		attack_frequency_range = [0,300]

		inflow = 1800

		emission_path = '/Volumes/My Passport for Mac/Traffic_attack_sim_results/Radar_warp/i24_random_sample'
		acc_penetration = 0.2
