...
def make_mal_driver_list(Attack_duration,Attack_frequency,attack_multiplier,num_comped_ACCs=2,num_human_drivers=35,num_ACC_drivers = 3):
	...
	for i in range(num_comped_ACCs):
		...
		label_adv = 'MVA_adv_DA_'+str(np.round(Attack_duration,2))+'_AF_'+str(np.round(Attack_frequency,2))+'_AM_'+str(np.round(attack_multiplier,2))
...
def get_file_name_no_version(attack_params):
	...
	attack_multiplier = attack_params[2]
	
	file_name_no_version = 'MVA_AD_'+str(Attack_duration)+'_AF_'+str(Attack_frequency)+'_AM_'+str(attack_multiplier)
...
def run_sim_with_attack(attack_params,emission_path,ring_length=600):
	...
	driver_controller_list_with_attack = make_mal_driver_list(Attack_duration,Attack_frequency,attack_multiplier)
...
if __name__ == '__main__':
	...
	emission_path = '/Volumes/My Passport for Mac/Traffic_attack_sim_results/Radar_warp/single_lane_ring_road_attack_monte_carlo'


	want_test_sim = False
	if(want_test_sim):

		attack_params = [np.random.uniform()*20,np.random.uniform()*60,-1 + np.random.uniform()*(4)]

		run_sim_with_attack(attack_params,emission_path)


	want_run_all_sims = True
	...
	if(want_run_all_sims):
		...
		for i in range(num_samples):
			attack_params_list[i,:] = np.array([np.random.uniform()*20,np.random.uniform()*60,-1 + np.random.uniform()*(4)])
