...
def replay(args,
    ...
    if args.controller:
        test_params = {}
        test_params['fail_safe'] = ['obey_speed_limit', 'safe_velocity', 'feasible_accel']
    ...
    if os.path.split(output_dir)[1] == args.exp_title:
        output_dir = os.path.join(output_dir, 'data.csv')
    else:
        output_dir = os.path.join(os.path.split(output_dir)[0], 'data.csv')
    with open(output_dir, 'a+') as writer:
        # Write header if new
        if os.stat(output_dir).st_size == 0:
            writer.write(','.join([key for key in info_dict.keys()]) + '\n')
        writer.write(','.join([str(np.mean(val)) for val in info_dict.values()]) + '\n')
    ...
    if args.run_transfer:
        ...
        ray_output = [
            replay.remote(
                args,
                flow_params,
                output_dir="{}/{}/{}".format(output_dir, 'idm_sweep', a),
                rllib_config=args.rllib_result_dir,
                max_completed_trips=args.max_completed_trips,
                default_controller_params={'idm_params': {'a': a}}
            )
            for a in np.arange(0.2, 2, 0.4)]
        ray.get(ray_output)
        ...
        ray_output = [
            replay.remote(
                args,
                flow_params,
                output_dir="{}/{}/{}".format(output_dir, 'inflow_sweep', inflow_rate),
                rllib_config=args.rllib_result_dir,
                max_completed_trips=args.max_completed_trips,
                inflow_rate=inflow_rate
            )
            for inflow_rate in range(1800, 2500, 200)]
        ray.get(ray_output)
        ...
        ray_output = [
            replay.remote(
                args,
                flow_params,
                output_dir="{}/{}/{}".format(output_dir, 'outflow_sweep', outflow_speed_limit),
                rllib_config=args.rllib_result_dir,
                max_completed_trips=args.max_completed_trips,
        ...
        ray_output = [
            replay.remote(
                args,
                flow_params,
                output_dir="{}/{}/{}".format(output_dir, 'lanefreq_sweep', lf),
                rllib_config=args.rllib_result_dir,
                max_completed_trips=args.max_completed_trips,
