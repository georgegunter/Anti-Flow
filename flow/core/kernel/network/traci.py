...
class TraCIKernelNetwork(BaseKernelNetwork):
    ...
    def __init__(self, master_kernel, sim_params):
        ...
        self.net_path = '/tmp/flow/debug/net/'
        self.cfg_path = '/tmp/flow/debug/cfg/'

        ensure_dir(self.net_path)
        ensure_dir(self.cfg_path)
    ...
    def generate_network(self, network):
        ...
        if self.network.net_params.template is not None:
            self.generate_net_from_template(self.network.net_params)
        elif self.network.net_params.osm_path is not None:
            self.generate_net_from_osm(self.network.net_params)
        else:
        ...
        for _ in range(RETRIES_ON_ERROR):
            try:
                edges, connections, road_grade = self._import_edges_from_net(
                    self.network.net_params)
                self._edges = edges
                self._connections = connections
                self._road_grade = road_grade
            except Exception as e:
                print('Error during start: {}'.format(e))
                print('Retrying in {} seconds...'.format(WAIT_ON_ERROR))
                time.sleep(WAIT_ON_ERROR)

        # If the above method failed to generate the proper edges, connections,
        # and road grades, raise the error that occurred.
        if self._edges is None and self._connections is None:
            raise error
    ...
    def generate_net(self,
        ...
        * connections (optional): describes how incoming and outgoing edge/lane
        ...
        if os.environ.get("USE_SINGULARITY", 0):
            net_cmd = "{} ".format(config.SINGULARITY_PATH) + net_cmd

        # Call the netconvert command.
        subprocess.call([net_cmd], stdout=subprocess.DEVNULL, shell=True)
    ...
    def _import_edges_from_net(self, net_params):
        ...
        for edge in root.findall('edge'):
            ...
            for i, lane in enumerate(edge):
                ...
                if i == 0:
                    ...
                    # Add the road grade data.
                    road_grade_data[edge_id] = self._get_road_grade_from_shape(
                        lane.attrib['shape'])
        ...
        return net_data, connection_data, road_grade_data
    ...
    def road_grade(self, edge, pos):
        ...
        float
            the road grade
        """
        return self._road_grade_util(self._road_grade.get(edge, []), pos)

    @staticmethod
    def _road_grade_util(rg_by_pos, pos):
        """Find the road grade from values at different positions on an edge.

        Parameters
        ----------
        rg_by_pos : list of (float, float)

        Returns
        -------
        float
            the road grade at the given position
        """
        if len(rg_by_pos) == 0:
            # No road grade was provided. Pass a default value.
            return 0

        # Choose the road grade of the first position you pass.
        for (pos_i, road_grade_i) in reversed(rg_by_pos):
            if pos > pos_i:
                return road_grade_i

        # This should not occur, but in case it does we pass a error.
        raise ValueError("Position {} smaller than expected".format(pos))

    @staticmethod
    def _get_road_grade_from_shape(shape):
        """Extract the road grade from the x,y,z coordinates in the shape.

        If no z-coordinates are provided for the given edge, and empty list is
        returned. The road grade at any point in the edge will subsequently
        default to 0.
        """
        # Separate the points in the shape.
        pos = shape.split(" ")
        for i in range(len(pos)):
            try:
                x, y, z = pos[i].split(",")
            except ValueError:
                # If z-coordinates are not provided,
                return []
        ...
        for i in range(1, len(pos)):
            ...
            # Compute the road grade.
            dist_i = np.sqrt((x1 - x0) ** 2 + (y1 - y0) ** 2)
            rg_by_pos.append((dist, (z1 - z0) / dist_i))
