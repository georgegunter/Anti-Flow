...
def sim_attack_smoother_general(
    v_des,
    ordering=None,
    random_AV_placement=True,
    want_render=False,
    emission_path='data',
    num_lanes = 1,
    ring_length = 500,
    sim_time = 200,
    num_human_drivers_per_lane = 38,
    Total_Attack_Duration=10,
    attack_decel_rate= -1.0):
    ...
    #human drivers:
    num_human_drivers = int(num_lanes * num_human_drivers_per_lane)
    ...
    for i in range(num_human_drivers):
        a = a_mean + np.random.normal(0,0.1)
        b = b_mean + np.random.normal(0,0.2)
        s0 = s0_mean + np.random.normal(0,0.2)
        s1 = s1_mean + np.random.normal(0,0.2)
        Vm = Vm_mean + np.random.normal(0,0.5)
    ...
    # follower-stopper:
    fstop_controller = (FollowerStopper,{'v_des':v_des})
    fstop_label = 'FollowerStopper_Vdes_'+str(v_des)
    ...
    # add AVs in the correct ordering:
    if((ordering is None) or (ordering.upper() == 'FOLLOWING')):
        driver_controller_list.append([fstop_label,fstop_controller,(StaticLaneChanger,{}),num_lanes])
        driver_controller_list.append([label_adv,adversary,(StaticLaneChanger,{}),num_lanes])
    else:
        driver_controller_list.append([label_adv,adversary,(StaticLaneChanger,{}),num_lanes])
        driver_controller_list.append([fstop_label,fstop_controller,(StaticLaneChanger,{}),num_lanes])
    ...
    sim_res_list = utils.run_ring_sim_variable_cfm(
    ...
    sim_results_path = os.path.join(os.getcwd(),sim_res_list[1])

    trajectory_dict = get_trajectory_timeseries(csv_path = sim_results_path)

    return trajectory_dict, sim_results_path
...
def get_losses_per_vehicle(trajectory_dict,time,model):
