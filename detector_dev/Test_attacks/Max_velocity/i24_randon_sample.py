...
def get_flow_params(attack_duration,
	...
	if(ACC_comp_params is not None):
		...
		adversary_accel_controller = (ACC_comp_overwrite_Vm,{
			'k_1':k_1,
			'k_2':k_2,
			'h':h,
			'd_min':d_min,
...
def rename_file(csv_path,emission_path,attack_duration,attack_magnitude,attack_frequency,acc_penetration,attack_penetration,inflow):
	...
	file_name_no_version = 'MVA_AD_'+str(attack_duration)+'_AF_'+str(attack_frequency)+'_MV_'+str(attack_magnitude)+'_Inflow_'+str(inflow)+'_ACCPenetration_'+str(acc_penetration)+'_AttackPenetration_'+str(attack_penetration)
...
if __name__ == '__main__':
	...
	if(want_random_sample):
		...
		attack_magnitude_range = [0,30.0]
		attack_duration_range = [0,20]
		attack_frequency_range = [0,300]

		inflow = 1800

		emission_path = '/Volumes/My Passport for Mac/Traffic_attack_sim_results/max_velocity/i24_random_sample'
		acc_penetration = 0.2
	...
	want_single_simulation = False

	if(want_single_simulation):

		print('Running single simulation.')

		emission_path = '/Volumes/My Passport for Mac/Traffic_attack_sim_results/max_velocity/i24_random_sample'
		
		attack_magnitude = 7.0
